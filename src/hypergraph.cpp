...
namespace hypergraph_reorder
{
    ...
    Hypergraph::Hypergraph()
        : n_nodes_(0), n_nets_(0), total_pins_(0) {}
    ...
    Hypergraph Hypergraph::from_clique_cover(const CliqueCover &cover)
    {
        ...
        std::cout << "Constructing CNH: " << hg.n_nodes_ << " nodes, "
                  << n_vertices << " vertices" << std::endl;
        ...
        for (index_t v = 0; v < n_vertices; ++v)
        {
            ...
            if (!cliques.empty())
            {
                ...
                // Deduplicate cliques (a vertex might appear in same clique multiple times due to graph structure)
                ...
                if (unique_cliques.size() < original_size)
                {
                    ...
                    if (v < 10)
                    { // Debug first few
                        std::cout << "Vertex " << v << ": removed " << (original_size - unique_cliques.size())
                                  << " duplicates (" << original_size << " -> " << unique_cliques.size() << ")" << std::endl;
                    }
                }

                hg.edges_.insert(hg.edges_.end(), unique_cliques.begin(), unique_cliques.end());
        ...
        {
            std::cout << "Removed " << total_duplicates << " duplicate clique entries total" << std::endl;
        }

        // Verification: check for duplicates in final hypergraph AND print first few nets
        ...
        for (index_t net = 0; net < std::min(hg.n_nets_, index_t(5)); ++net)
        {
            ...
            std::vector<index_t> pins(hg.edges_.begin() + start, hg.edges_.begin() + end);
            ...
            if (unique_count < pins.size())
            {
                ...
                std::cout << "  -> HAS DUPLICATES: " << pins.size() << " -> " << unique_count << std::endl;
            }
        }
        if (verification_duplicates > 0)
        {
            std::cout << "WARNING: Found " << verification_duplicates << " nets with duplicate pins in final hypergraph!" << std::endl;
        }

        std::cout << "CNH constructed: " << hg.n_nets_ << " nets (efficiency: "
                  << (100.0 * hg.n_nets_ / n_vertices) << "%)" << std::endl;
