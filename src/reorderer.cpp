...
namespace hypergraph_reorder {
 ...
 SymmetricDBReorderer::SymmetricDBReorderer(const Options& opts)
    : opts_(opts) {
...
SymmetricDBReorderer::Result SymmetricDBReorderer::reorder(const CSRMatrix& matrix) {
    Result result;
    Timer timer;
    ...
    result.stats.separator_ratio = vertex_partition.separator_ratio(graph.n_vertices());
...
HypergraphPartition SymmetricDBReorderer::partition_hypergraph(const Hypergraph& hg) {
...
VertexPartition SymmetricDBReorderer::create_vertex_partition(
    ...
    index_t n_vertices) {
    return partitioner_->create_vertex_partition(cnh_partition, cover, n_vertices);
}
...
std::vector<index_t> SymmetricDBReorderer::create_permutation(
    ...
    perm.insert(perm.end(), partition.separator.begin(), partition.separator.end());
