...
namespace hypergraph_reorder
{
        ...
        : n_cliques_(cliques.size()), n_vertices_(n_vertices), max_clique_size_(0)
    {
        ...
        for (size_t i = 0; i < cliques.size(); ++i)
        {
            ...
            max_clique_size_ = std::max(max_clique_size_, static_cast<index_t>(cliques[i].size()));
        }
        ...
        {
            vertex_cliques_ptr_[v + 1] = vertex_cliques_ptr_[v] + vertex_to_cliques[v].size();
        }
    ...
    std::span<const index_t> CliqueCover::get_vertex_cliques(index_t vertex_id) const
    {
    ...
    CliqueCoverSolver::CliqueCoverSolver(const Options &opts)
        : opts_(opts) {}
    ...
    CliqueCover CliqueCoverSolver::solve(const Graph &graph)
    {
        ...
        if (opts_.use_maximal_cliques && !opts_.greedy_only)
        {
            if (graph.n_vertices() <= opts_.max_clique_enum_vertices)
            {
                ...
                std::cout << "Found " << all_cliques.size() << " maximal cliques" << std::endl;
            ...
            else
            {
                ...
                std::cout << "Graph too large for full maximal clique enumeration" << std::endl;
                std::cout << "Using greedy triangle selection instead..." << std::endl;
                auto all_triangles = enumerate_triangles(graph);
                std::cout << "Found " << all_triangles.size() << " triangles" << std::endl;
...
#ifdef USE_OPENMP
        int num_threads = opts_.num_threads > 0 ? opts_.num_threads : omp_get_max_threads();
...
#pragma omp for schedule(dynamic, 64)
            ...
            for (index_t u = 0; u < graph.n_vertices(); ++u)
            {
                ...
                for (auto v : u_neighbors)
                {
                    ...
                    {
                        if (w <= v)
                            continue; // Ensure u < v < w

                        // Check if w is also a neighbor of v
                        if (std::find(v_neighbors.begin(), v_neighbors.end(), w) != v_neighbors.end())
                        {
...
#pragma omp for schedule(dynamic, 1)
        ...
        std::vector<std::vector<index_t>> &cliques)
    {
        ...
        if (P.empty() && X.empty())
        {
            ...
            if (R.size() >= 2)
            { // Only keep cliques with at least 2 vertices
                cliques.push_back(R);
            }
            return;
        }
        ...
        for (const auto &[size, idx] : clique_sizes)
        {
            ...
            for (size_t i = 0; i < clique.size(); ++i)
            {
                for (size_t j = i + 1; j < clique.size(); ++j)
                {
                    ...
                    if (graph.has_edge(u, v) && covered_edges.find({u, v}) == covered_edges.end())
                    {
                        uncovered_count++;
                    }
                }
        ...
        std::vector<std::vector<index_t>> &cliques)
    {
        ...
        std::cout << "Added " << edge_set.size() << " 2-cliques for uncovered edges" << std::endl;
    }
