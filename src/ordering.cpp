...
BlockOrderer::BlockOrderer(const Options& opts)
    : opts_(opts) {}
...
BlockOrderingResult BlockOrderer::apply_ordering(
    ...
    if (opts_.use_parallel) {
        ...
        std::cout << "Using " << num_threads << " threads for parallel ordering" << std::endl;
    }
#endif
    ...
    for (index_t part_id = 0; part_id < partition.n_parts; ++part_id) {
        ...
        try {
            ...
            // Update partition (thread-safe since each thread writes to different block)
    ...
    std::cout << "Block ordering completed: "
              << result.successful_blocks << " successful, "
              << result.failed_blocks << " failed" << std::endl;
...
std::vector<index_t> BlockOrderer::compute_block_ordering(
    const CSRMatrix& matrix,
    const std::vector<index_t>& block_vertices) {
    ...
    for (index_t i = 0; i < block_size; ++i) {
        ...
        auto global_vals = matrix.pattern_only() ? std::span<const value_t>() : matrix.row_values(global_row);
...
std::vector<index_t> BlockOrderer::compute_cholmod_ordering(
    ...
    for (index_t i = 0; i < n; ++i) {
        ...
        auto vals = block_matrix.pattern_only() ? std::span<const value_t>() : block_matrix.row_values(i);
