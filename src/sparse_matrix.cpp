...
namespace hypergraph_reorder
{
    ...
    CSRMatrix::CSRMatrix()
        : n_rows_(0), n_cols_(0), nnz_(0),
          is_symmetric_(false), pattern_only_(false) {}
    ...
    CSRMatrixView CSRMatrix::get_block(const std::vector<index_t> &row_indices) const
    {
    ...
    {
        return CSRMatrix(n_rows_, n_cols_,
    ...
    CSRMatrix CSRMatrix::extract_upper_triangle() const
    {
        if (n_rows_ != n_cols_)
        {
            throw HypergraphReorderError("Can only extract upper triangle from square matrix");
        }
        ...
        if (!pattern_only_)
            upper_values.reserve(upper_nnz);
        ...
        return CSRMatrix(n_rows_, n_cols_,
                         std::move(upper_row_ptr),
                         std::move(upper_col_idx),
                         std::move(upper_values),
                         false);
    }
                                  ...
                                  bool is_symmetric)
    {
        ...
        return CSRMatrix(n_rows, n_cols,
                         std::move(row_ptr),
                         std::move(col_idx),
                         std::move(values),
                         is_symmetric);
                             ...
                             const std::vector<index_t> &perm)
    {
        ...
        for (index_t old_i = 0; old_i < n; ++old_i)
        {
            ...
            auto old_vals = matrix.pattern_only() ? std::span<const value_t>() : matrix.row_values(old_i);
