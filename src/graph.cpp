...
namespace hypergraph_reorder
{
    ...
    Graph::Graph()
        : n_vertices_(0), n_edges_(0) {}

    Graph::Graph(index_t n_vertices)
        : n_vertices_(n_vertices), n_edges_(0)
    {
        adj_ptr_.resize(n_vertices + 1, 0);
    }

    Graph::Graph(index_t n_vertices, index_t n_edges,
                 std::vector<index_t> adj_ptr,
                 std::vector<index_t> adj_list)
        : n_vertices_(n_vertices), n_edges_(n_edges),
    ...
    Graph Graph::from_symmetric_matrix(const CSRMatrix &matrix)
    {
        ...
        {
            throw HypergraphReorderError("Matrix must be square for graph construction");
        }
        ...
        for (index_t i = 0; i < n; ++i)
        {
            std::sort(adj_list.begin() + adj_ptr[i],
                      adj_list.begin() + adj_ptr[i + 1]);
        }
    ...
    {
        if (adj_ptr_.size() != static_cast<size_t>(n_vertices_ + 1))
            return false;
        ...
        for (index_t i = 0; i < n_vertices_; ++i)
        {
            ...
            {
                if (neighs[k] < 0 || neighs[k] >= n_vertices_)
                    return false;
                if (neighs[k] == i)
                    return false;
                if (k > 0 && neighs[k] <= neighs[k - 1])
                    return false;
            }
        }
