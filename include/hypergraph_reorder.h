...
int hgr_reorder_file(hgr_reorderer_t* reorderer,
                     const char* input_path,
                     hgr_matrix_format_t format,
                     hgr_result_t** result);

/* Reorder from CSR matrix in memory */
int hgr_reorder_csr(hgr_reorderer_t* reorderer,
...
/* Extract results */
int hgr_get_permutation(hgr_result_t* result,
                        const int64_t** permutation,
                        int64_t* n);

int hgr_get_reordered_matrix(hgr_result_t* result,
                             int64_t* n_rows, int64_t* nnz,
                             const int64_t** row_ptr,
                             const int64_t** col_idx,
                             const double** values);
...
/* Save result to file */
int hgr_save_matrix(hgr_result_t* result,
                    const char* output_path,
                    hgr_matrix_format_t format);

/* Get statistics */
int hgr_get_statistics(hgr_result_t* result,
